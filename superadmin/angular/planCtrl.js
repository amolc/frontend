app.controller("planCtrl", function($scope, $http, $window, $location, $sce, $timeout, store, config) {
    ...
    $scope.add = function(req, res) {
        ...
        if (typeof $scope.data.id == 'undefined') {
            ...
            $http.post(config.baseurl + 'plan/', $scope.data)
                .success(function(res) {
                    if (res.status == 'false') {} else {
            ...
            console.log($scope.data);
            $http.patch(config.baseurl + 'plan/', $scope.data)
                .success(function(res) {
                    if (res.status == 'false') {} else {
                        $scope.response = res.data;
                        console.log('message: ', $scope.response);
    ...
    $scope.update = function(id) {
        $http.get(config.baseurl + 'plan/' + id)
            .success(function(res) {
                if (res.status == 'false') {} else {
                    $scope.data = res.data;
    ...
    $scope.delete = function(id) {
        ...
        $http.delete(config.baseurl + 'plan/' + id + '/')
            .success(function(res) {
                if (res.status == 'false') {} else {
                    $scope.response = res.data;
